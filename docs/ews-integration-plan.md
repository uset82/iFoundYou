# EWS Integration Plan — Apocalypse Early Warning System in iFoundYou

## Overview

Integrate the **Apocalypse Early Warning System (EWS)** features from `ews-main` into the **iFoundYou** app so users can see real-time emergency levels, airborne aircraft counts, and anomaly signals directly inside the iFoundYou "Inside Alerts" experience.
...
- [x] **3.1** Create `web/src/components/EWS/AircraftOverlay.tsx` — a MapLibre layer that renders aircraft positions from the EWS live snapshot on the existing iFoundYou map
...
- [x] **4.1** Add an "Apocalypse EWS" tab/section inside the existing iFoundYou alerts view (`view === 'alerts'`)
...
- [x] **6.1** When EWS emergency level hits 5, auto-create a community alert in iFoundYou (category: "other", message: "⚠️ Apocalypse EWS Level 5 — Unusual aircraft activity detected")
...
- [x] **7.4** Add responsive design for the EWS components (mobile-first, works in the existing iFoundYou layout)
...
- **No backend changes needed for iFoundYou** — EWS data is consumed from the public R2 JSON endpoints (read-only, CORS-enabled)
- **Shared MapLibre instance** — Aircraft overlay reuses the existing map in iFoundYou rather than creating a second map
- **Polling model** — Dashboard JSON is polled every 5 minutes (matching EWS refresh cadence)
- **TypeScript** — All new code follows iFoundYou's existing TS + React stack
...
1. User can see the current EWS emergency level (1–5) inside iFoundYou
